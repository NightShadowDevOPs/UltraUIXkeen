...
## Быстрое подключение
1. Убедись, что router-agent на роутере обновлён минимум до **0.6.33**.
...
- Релиз `v1.2.141` не меняет структуру JSON для Home Assistant.
- Если YAML уже переведён на `ha_snapshot`, дополнительных правок по структуре сейчас не требуется.
