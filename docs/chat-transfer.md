...
Текущая версия архива: v1.2.21
router-agent: 0.6.8

Последний фикс в этом релизе:
- таблица `Трафик` теперь подтягивает сохранённые лимиты и сохранённый QoS не только из текущего traffic bucket, но и из router `qos_status`, Source IP mapping и уже сохранённых limit owners
- строки с сохранёнными лимитами/QoS больше не теряются из-за top-N фильтра и отсутствия текущего трафика: важные устройства остаются видимыми
- действия по лимитам и shaping теперь используют фактического владельца лимита, даже если строка сейчас отображается по IP, а лимит сохранён под именем устройства
