...
Текущая версия архива: v1.2.23
router-agent: 0.6.8

Последний фикс в этом релизе:
- QoS Apply/Clear и re-apply shaping больше не зависят от застрявших browser-local флагов router-agent; UI принудительно поднимает их на старте и при самих действиях
- Source IP labels со scope из другого ПК/браузера больше не считаются «чужими» только из-за локальных UUID backend'ов, поэтому имена устройств, лимиты и QoS снова корректно связываются в таблице `Трафик` между ПК
- hard bandwidth shaping теперь переактивируется не только при сохранении лимита, но и при ручном re-apply / refresh MAC, чтобы шейпер реально резал канал, а не только хранил значение
...
- UI v1.2.23 / agent 0.6.8: исправлен реальный корень с меж-ПК `Трафик`/QoS — локальные browser-local toggles router-agent теперь повторно мигрируются в `on`, а QoS Apply/Clear и re-apply shaping больше не глушатся ими в интерфейсе. Плюс Source IP labels со scope, пришедшим с другого ПК, перестали считаться «чужими» только из-за локальных UUID backend'ов, поэтому имена устройств, сохранённые лимиты и QoS снова корректно сцепляются в одной таблице между разными ПК.
- UI v1.2.22 / agent 0.6.8: локальные browser-тогглы router-agent теперь автоподнимаются на новом ПК/в новом профиле, поэтому общие лимиты/QoS из роутера и users-db снова подгружаются без отдельного ручного включения. Сохранение лимита канала теперь автоматически включает router shaping enforcement, чтобы шейпер реально применялся, а не только сохранялся в UI.
- UI v1.2.21 / agent 0.6.8: таблица `Трафик` теперь держит строки с сохранёнными лимитами и QoS даже без текущего трафика, подмешивает IP из router `qos_status`, а действия лимитов/shaping привязывает к фактическому stored limit owner, чтобы состояния, сделанные вчера или на другом ПК, не терялись из вида.
- UI v1.2.20 / agent 0.6.8: исправлен резолв IP в строках `Трафик`, из-за чего кнопка `Применить` в QoS снова работает для устройств/пользователей, пришедших из Source IP mapping, live connections и legacy traffic buckets.
