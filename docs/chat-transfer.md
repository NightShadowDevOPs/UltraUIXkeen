...
Текущая версия архива: v1.1.116
router-agent: 0.5.59

Последний фикс в этом релизе:
- добавлен компактный статус синхронизации store трафика провайдеров в Tasks / Dev panel: видно состояние sync, ревизию, время последнего pull/push и можно форсировать refresh/push
- исправлена логика `latest-version` router-agent: удалён слишком широкий парсинг `version` из install.sh, теперь версия тянется по явному `AGENT_VERSION`, а в статусе latest никогда не показывается ниже локально установленной
- хранение агрегированных счётчиков трафика провайдеров на стороне router-agent из v1.1.115 сохранено; счётчики `Сегодня` и `С момента сброса` продолжают переживать рестарты UI / agent / роутера
- localStorage в UI остаётся fallback-кэшем; запись на роутер по-прежнему идёт редкой синхронизацией и merge по максимальным значениям, чтобы не долбить flash на каждый тик
