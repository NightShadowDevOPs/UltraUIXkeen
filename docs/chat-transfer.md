...
Текущая версия архива: v1.2.15
router-agent: 0.6.8

Последний фикс в этом релизе:
- в списке устройств под живым трафиком добавлены компактные QoS-индикаторы с 6-уровневой шкалой, чтобы сразу видеть приоритет устройства без захода в отдельную карточку QoS
- те же визуальные индикаторы добавлены и в сам список `Host QoS priority`, чтобы профиль читался не только текстом, но и значком/уровнем
- список устройств теперь подтягивает актуальный `qos_status` от router-agent отдельным фоном, поэтому бейджи/уровни обновляются и после применения профиля
