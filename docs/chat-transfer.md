...
Текущая версия архива: v1.2.16
router-agent: 0.6.8

Последний фикс в этом релизе:
- Host QoS теперь сохраняет выбранные/применённые профили в UI при переходах по меню на том же ПК, чтобы настройки не исчезали после обычной навигации
- список устройств под живым трафиком реально подмешивает `qos_status` в активные host rows и показывает заметные QoS-значки, а не только тонкие полоски
- список `Host QoS priority` тоже получил более явные иконки уровня, чтобы applied priority читался сразу
