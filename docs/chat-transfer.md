...
Текущая версия архива: v1.2.4
router-agent: 0.6.1

Последний фикс в этом релизе:
- на вкладке `Подписки` вместо расплывчатого `Копировать лучший URL` теперь явно показывается, какой источник реально используется сейчас: локальный router-agent URL или внешний HTTPS URL
- для Mihomo/Clash и V2Ray/Xray добавлены отдельные статусные плашки `Сейчас используется`, чтобы было сразу видно, откуда берутся copy/QR/deeplink
- русские подписи в блоке публикации и JSON preview выровнены: `Внешняя HTTPS-база (необязательно)`, `Локальный JSON URL (предпросмотр)`, `Копировать текущий URL`
