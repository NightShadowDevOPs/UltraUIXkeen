...
- verify real-router behavior after `v1.2.169`:
  1. `Трафик` shows only one primary top-level mode switcher
  2. drill-down focus from device/user cards is visible immediately and can be reset in one click
  3. switching between `Устройства` and `Пользователи` inside a focused state still feels predictable
