#!/opt/bin/sh
# UI Mihomo Ultra v1.2.177 — concise zash-agent smoke checks.
...
TMP_DIR="/tmp/zash-agent-check-v1.2.177.$$"
WGET="/opt/bin/wget"
[ -x "$WGET" ] || WGET="wget"
...
fetch_cmd HA_STATUS ha_status 12
fetch_cmd HA_TRAFFIC ha_traffic 12
fetch_cmd HA_USERS ha_users 15
fetch_cmd HA_QOS ha_qos 15
fetch_cmd HA_SNAPSHOT ha_snapshot 15
fetch_cmd MIHOMO_PROVIDERS mihomo_providers 15

if [ -f /opt/zash-agent/var/ha-strict.log ]; then
  echo "HA_STRICT_LOG_EXISTS=true"
  echo "HA_STRICT_RECENT_LEAKS=$(grep -c 'strict_.*=true' /opt/zash-agent/var/ha-strict.log 2>/dev/null || echo 0)"
else
  echo "HA_STRICT_LOG_EXISTS=false"
  echo "HA_STRICT_RECENT_LEAKS=0"
fi

echo
echo '=== result hint ==='
echo 'PASS when STATUS_HTTP=200 and all HA_*_HTTP=200/HA_*_OK=true. HA_STRICT_RECENT_LEAKS may be >0 immediately after fixing old sort stdout leaks; endpoints must still return valid HTTP 200 JSON.'
