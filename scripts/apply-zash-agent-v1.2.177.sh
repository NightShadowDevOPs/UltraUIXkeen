#!/opt/bin/sh
# UI Mihomo Ultra v1.2.177 — scoped zash-agent patch installer.
...
CHECK_SCRIPT="$RELEASE_DIR/scripts/check-zash-agent-v1.2.177.sh"
AGENT_DIR="/opt/zash-agent"
PID_FILE="$AGENT_DIR/var/httpd.pid"
STAMP="$(date +%Y%m%d-%H%M%S 2>/dev/null || echo now)"
BACKUP="/opt/zash-agent.backup-v1.2.177-$STAMP.tar.gz"
SH_BIN="/opt/bin/sh"
...
echo "[zash-agent-patch] release=v1.2.177"
