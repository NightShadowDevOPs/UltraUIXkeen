#!/opt/bin/sh
# UI Mihomo Ultra v1.2.177 — restore /opt/zash-agent from the latest patch backup.
...
KEEP_CURRENT="/opt/zash-agent.rollback-before-v1.2.177-$STAMP.tar.gz"

BACKUP="${1:-}"
if [ -z "$BACKUP" ]; then
  BACKUP="$(ls -1t /opt/zash-agent.backup-v1.2.177-*.tar.gz /opt/zash-agent.backup-v1.2.175-*.tar.gz 2>/dev/null | head -n 1)"
fi
