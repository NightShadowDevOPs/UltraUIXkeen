...
Текущая версия архива: v1.2.12
router-agent: 0.6.7

Последний фикс в этом релизе:
- добавлен groundwork для `Host QoS priority`: на странице роутера появился отдельный блок с LAN-хостами, живыми скоростями и профилями High / Normal / Low
- router-agent 0.6.7 получил новые команды `qos_status`, `qos_set`, `qos_remove` и умеет переживать перезапуск с сохранением QoS-профилей в отдельной БД
- hard shaping по IP не убран: если у одного и того же IP одновременно есть жёсткий лимит и QoS-профиль, приоритет остаётся за жёстким лимитом, чтобы поведение было предсказуемым
