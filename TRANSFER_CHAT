...
Текущая версия архива: v1.2.22
router-agent: 0.6.8

Последний фикс в этом релизе:
- локальные browser-тогглы router-agent теперь автоподнимаются на новом ПК/в новом профиле, поэтому общие лимиты/QoS и users-db sync подхватываются без отдельного ручного включения
- при сохранении лимита канала UI автоматически включает router-side shaping enforcement, чтобы шейпер реально применял ограничение, а не просто сохранял число в настройках
