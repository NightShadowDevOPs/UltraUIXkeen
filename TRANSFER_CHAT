...
Текущая версия архива: v1.2.19
router-agent: 0.6.8

Последний фикс в этом релизе:
- добавлен отдельный верхнеуровневый раздел `Трафик` в левом меню; управление трафиком устройств, лимитами и QoS теперь собрано в одном месте
- страница `Пользователи` больше не дублирует таблицу трафика и показывает только Source IP mapping + карточку-переход в новый раздел `Трафик`
- таблица `Трафик по пользователям` получила встроенную колонку QoS: текущий профиль, выбор профиля и apply/clear прямо в строке устройства/пользователя
- QoS применяется ко всем IP, которые сейчас связаны с этим устройством/пользователем; router-agent в этом релизе не менялся и остаётся 0.6.8
