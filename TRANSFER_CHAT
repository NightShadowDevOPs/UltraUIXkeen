...
Текущая версия архива: v1.2.20
router-agent: 0.6.8

Последний фикс в этом релизе:
- исправлен резолв IP в колонке QoS на странице `Трафик`: кнопка `Применить`/`Очистить` снова активна для строк, которые собирались не из прямого IP-ключа, а из label/legacy traffic bucket
- для QoS-строки теперь дополнительно используются уже существующие Source IP mappings и живые подключения, поэтому таблица лучше находит текущий IP устройства/пользователя
