...
Текущая версия архива: v1.1.117
router-agent: 0.5.59

Последний фикс в этом релизе:
- в карточке live-трафика роутера добавлено отдельное разделение внешнего трафика на `VPN / туннели` и `Bypass`; теперь вместо одного общего блока `вне Mihomo` видно, какая часть ушла в туннели, а какая проходит напрямую
- диаграмма live-трафика теперь показывает отдельные агрегированные серии `VPN ↓/↑` и `Bypass ↓/↑`, при этом детальные серии по конкретным tunnel-интерфейсам сохранены ниже как и раньше
- таблица активных хостов стала нагляднее: добавлен явный столбец контура/типа (`Mihomo`), чтобы было видно, где атрибуция по устройствам честная, а где роутер пока может показать только агрегированный поток
- status sync provider-traffic store и фикс latest-version router-agent из v1.1.116 сохранены без изменений
