...
- router-agent `status` payload is cached for a short TTL and cleared after shaping changes, so multiple router cards do not keep recomputing the same expensive diagnostics in parallel
- frontend agent API now coalesces short-lived `status` / `qos_status` requests, reducing duplicate polling pressure when several router cards refresh together

## v1.2.111 — 2026-03-31
- router UI optimization hotfix: hidden `Router` sections now unmount instead of staying alive with background polling
- reduced live polling pressure on visible router cards: `SystemCard`, `AgentCard`, host QoS, and user QoS refresh now run less often
- live polling now pauses on hidden browser tabs for router traffic, router agent status, QoS, and Tasks logs/upstream checks
- `NetcrazeTrafficCard` polling intervals were relaxed to reduce router CPU churn while preserving live visibility for traffic and host activity
- router-agent version did not change and remains `0.6.24`
...
- обзор показывает популярные верхнеуровневые секции и ключевые параметры без ручного чтения всего YAML: режим, логирование, LAN/IPv6, порты, controller, secret, geodata, наличие модулей `tun / dns / profile / sniffer`
- отдельно считаются объёмы маршрутизации: число `proxies`, `proxy-groups`, `rules`, `proxy-providers`, `rule-providers`, а также список секций верхнего уровня
- `router-agent` в этом релизе не менялся: остаётся линия `0.6.21`; новый обзор работает полностью на стороне UI поверх уже существующих источников `active / draft / baseline / editor`
- обновлены `docs/mihomo-config-management.md`, `docs/chat-transfer.md` и корневой `TRANSFER_CHAT`
...
- `Mihomo → Конфигурация`: можно выбирать левую и правую сторону сравнения, быстро менять их местами и при желании показывать только изменённые строки
- diff работает целиком на стороне UI поверх уже существующих managed-config API router-agent; версия `router-agent` в этом релизе не менялась
- обновлены `docs/mihomo-config-management.md`, `docs/chat-transfer.md` и корневой `TRANSFER_CHAT` под новый рабочий сценарий сравнения перед `validate/apply`
