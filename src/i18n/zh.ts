...
const zh: LANG_MESSAGE = {
  ...
  note: '提示',
  userEgressProviders: '提供商',
  userEgressSelectProviders: '允许的出口 proxy-providers',
  userEgressNoProviders: '提供商列表尚未加载（打开 Proxies/Providers 页面或等待刷新）。',
  userEgressRouting: '按用户出口（egress）',
  userEgressRoutingTip: '限制某个 LAN 客户端可使用哪些 proxy-providers。UI 会生成 YAML 片段：proxy-groups（use:）+ SRC-IP-CIDR 规则。',
  userEgressRoutingNone: '没有配置 egress 的用户（在 Source IP 映射行里选择提供商）。',
  userEgressRoutingGroups: '添加到 proxy-groups',
  userEgressRoutingRules: '添加到 rules',
  userEgressRoutingApplyTip: '把片段粘贴到主 config.yaml（Proxy-Groups / Rules）并 reload/restart Mihomo。SRC-IP-CIDR 规则需要放在更通用规则之前。',
  userEgressWarnRegex: 'key 是正则；egress 需要 IP/CIDR（SRC-IP-CIDR）',
  userEgressWarnUnsupported: 'key 不像 IP/CIDR — 已跳过',
  userEgressWarnMissingProviders: '当前提供商列表中缺失：',
  importLanHosts: '导入名称',
