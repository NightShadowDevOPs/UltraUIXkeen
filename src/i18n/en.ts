const en = {
  ...
  note: 'Note',
  userEgressProviders: 'Providers',
  userEgressSelectProviders: 'Allowed egress proxy-providers',
  userEgressNoProviders: 'Providers list is not loaded yet (open Proxies/Providers page or wait for refresh).',
  userEgressRouting: 'Per-user egress',
  userEgressRoutingTip: 'Restrict which proxy-providers a LAN client may use. The UI generates YAML snippets: proxy-groups (use:) + SRC-IP-CIDR rules.',
  userEgressRoutingNone: 'No users with egress configured (select providers in Source IP mapping rows).',
  userEgressRoutingGroups: 'Add to proxy-groups',
  userEgressRoutingRules: 'Add to rules',
  userEgressRoutingApplyTip: 'Paste the snippets into your main config.yaml (Proxy-Groups / Rules) and reload/restart Mihomo. SRC-IP-CIDR rules must be above more generic rules.',
  userEgressWarnRegex: 'key is a regex; egress needs IP/CIDR (SRC-IP-CIDR)',
  userEgressWarnUnsupported: 'key does not look like IP/CIDR — skipped',
  userEgressWarnMissingProviders: 'missing in current providers list:',
