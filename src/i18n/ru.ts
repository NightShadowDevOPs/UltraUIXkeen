...
const ru: LANG_MESSAGE = {
  ...
  note: 'Примечание',
  userEgressProviders: 'Провайдеры',
  userEgressSelectProviders: 'Разрешённые провайдеры для выхода (egress)',
  userEgressNoProviders: 'Список провайдеров ещё не загружен (откройте страницу Proxies/Providers или дождитесь обновления).',
  userEgressRouting: 'Egress по пользователям',
  userEgressRoutingTip: 'Можно задать, через каких proxy-providers может выходить конкретный LAN-клиент. UI генерирует фрагмент YAML: группы (proxy-groups с use:) + правила SRC-IP-CIDR.',
  userEgressRoutingNone: 'Нет пользователей с настроенным egress (выберите провайдеры у нужных записей в Source IP mapping).',
  userEgressRoutingGroups: 'Добавить в proxy-groups',
  userEgressRoutingRules: 'Добавить в rules',
  userEgressRoutingApplyTip: 'Вставьте блоки в основной config.yaml (Proxy-Groups / Rules) и сделайте reload/restart Mihomo. Правила SRC-IP-CIDR должны быть выше остальных правил, иначе их перекроют более общие правила.',
  userEgressWarnRegex: 'ключ — regex, для egress нужен IP/CIDR (SRC-IP-CIDR)',
  userEgressWarnUnsupported: 'ключ не похож на IP/CIDR — пропущено',
  userEgressWarnMissingProviders: 'не найдены в текущем списке:',
