...
Дополнение по v1.2.70
- router-agent hotfix v1.2.68-v1.2.69 признан неудачным для shell payload на роутере и откатан на чистую рабочую базу.
- В v1.2.70 агент переиздан как 0.6.17 на базе стабильного payload без синтаксических ошибок.
- Остаточный шум host-traffic temp files будет исправляться отдельным следующим шагом уже от стабильной базы.
